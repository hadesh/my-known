...
public actor QwenProvider: LLMProvider {
    ...
    public init(
        ...
        defaultModel: String = "qwen-turbo",
        embeddingModel: String = "text-embedding-v1"
    ) {
        self.name = name
        self.apiKey = apiKey
    ...
    public func embedding(text: String, model: String) async throws -> [Float] {
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              ...
              let embeddings = output["embeddings"] as? [[String: Any]],
              let firstEmbedding = embeddings.first,
              let embeddingArray = firstEmbedding["embedding"] as? [Double] else {
            throw LLMProviderError.decodingError(DecodingError.dataCorrupted(
                DecodingError.Context(codingPath: [], debugDescription: "Failed to decode Qwen embedding response")
            ))
        }
