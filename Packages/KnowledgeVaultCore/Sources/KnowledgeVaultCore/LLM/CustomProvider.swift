...
public actor CustomProvider: LLMProvider {
    ...
    public func embedding(text: String, model: String) async throws -> [Float] {
        guard supportsEmbeddings else {
            throw LLMProviderError.apiError("Custom provider '\(name)' does not support embeddings")
        }
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw LLMProviderError.decodingError(DecodingError.dataCorrupted(
                DecodingError.Context(codingPath: [], debugDescription: "Failed to decode custom provider embedding response")
            ))
        }
